...
4. Extended tests pass partial dependency and simultaneous contradiction/unknown handling. Across seven authored cases the full contract has zero unsafe permits and zero false refusals, versus 3/0 for simple voting and 1/0 for confidence-only and provenance-only. Cross-claim dependency visibility and reachable `DEGRADE` semantics remain gaps. Next define those plus property-typed failure domains before a stronger conservative-fusion comparison or larger build; see `validation/2026-09-04_EXTENDED_FALSIFIER_RESULTS.md`.
