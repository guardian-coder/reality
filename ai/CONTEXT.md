...
17. External learning is now an active parallel track. The first institutional brief, ranked shortlist, and human-written message draft are under `outreach/`. Contact is for falsification and technical guidance, not endorsement, funding, sales, or access to sensitive systems. No institution has been contacted yet.
18. A public narrative website was selected as the place to show the research journey and make every major claim traceable to evidence. The first build attempt on 2026-09-05 generated a partial local `site/` scaffold, but dependency installation was blocked by the workspace security policy (`EALLOWSCRIPTS`). The policy was not bypassed; the unverified scaffold remains uncommitted and no deployment was created.
