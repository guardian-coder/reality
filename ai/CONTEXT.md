...
9. Paired forcing cases now distinguish irrelevant versus claim-relevant shared failures and informational versus blocking cross-claim dependencies. Current string equality falsely revalidates T-01, while visibility-only incorrectly permits X-02 under its hypothetical joint policy. Next freeze and implement only the minimal typed dependency and explicit joint-claim rule described in `validation/2026-09-04_DEPENDENCY_SEMANTICS_FORCING_CASES.md`.
