...
const chapters: StoryChapter[] = [
  {
    ...
    guideNote: 'The first breakthrough was not an answer. It was refusing to let the current founder constraint define the size of the question.',
    status: 'DECISION',
  },
  {
    ...
    guideNote: '“Cheap” is a consequence, not the thesis. The driver is expanding capability and access.',
    status: 'WORLD MODEL',
  },
  {
    ...
    guideNote: 'The thesis survived the thought experiment—but only as a narrower hypothesis, not as proof of a product or market.',
  ...
  },
  {
    ...
    guideNote: 'Three messages can still be one observation wearing three uniforms.',
    status: 'CONNECTION',
  },
  {
    ...
    guideNote: 'The evaluator caught failures—and then became evidence of the same failure pattern it was designed to prevent.',
    status: 'PROTOTYPE',
  },
  {
    ...
    guideNote: 'This is the creative leap of the project. It remains a testable hypothesis, not a discovered law.',
  ...
  },
  {
    ...
    guideNote: 'A polished demo is still a demo. Grounding begins when someone outside the project can prove us wrong.',
    status: 'MARKET GATE',
  },
  {
    id: 'economics', number: '08', short: 'Economics', marker: 'OUTCOME-LINKED RESOURCE ACCOUNTING',
    title: 'Evidence became an economic control problem.',
    lead: 'Agents consume models, tools, APIs, money, and time before anyone knows whether the work created an accepted outcome.',
    observation: 'Generic cost dashboards count consumption. They do not reliably bind a task contract, pre-action permission, execution, external outcome evidence, and accepted value into one inspectable record.',
    inference: 'The Reality evidence foundation can support an Agent Economics control layer: decide whether an action may spend, then count value only after the required outcome evidence arrives.',
    next: 'Can an outside agent integrate this control without our help and complete one real economic chain?',
    guideNote: 'This does not replace agent runtimes. It governs the boundary where their proposed actions become economic commitments.',
    status: 'PRODUCT DIRECTION',
  },
  {
    id: 'integration', number: '09', short: 'V1 correction', marker: 'ENGINE TO USABLE PRODUCT',
    title: 'An API is not yet an integration experience.',
    lead: 'The first V1 implemented a real authorization engine, but the interface mostly displayed endpoints and records. A customer could see the control plane without knowing how to enter it.',
    observation: 'The product test failed at the first human boundary: no guided credential flow, no editable policy contract, no visible live verification path, and the research story occupied the front door.',
    inference: '“Done” must mean an unaided customer can connect a runtime, define a policy, test a controlled action, submit outcome evidence, and inspect the resulting record.',
    next: 'Will the corrected journey work with a real external runtime and a real outcome source, not only the built-in verification path?',
    guideNote: 'This correction is part of the evidence. We keep the failed completion claim because it shows exactly where engine-building stopped short of product-building.',
    status: 'BUILDING',
  },
];
...
const evidenceLedger = [
  ...
  { label: 'Agent Economics engine', value: 'Live V1', note: 'Pre-action permits and outcome records', tone: 'good' },
  { label: 'Unaided integration', value: 'Testing', note: 'Product journey corrected after first failure', tone: 'warn' },
    ...
    <main className="story-site">
      {guideOpen && (
        <dialog open className="guide-layer" aria-labelledby="guide-title">
          <button className="guide-dismiss" onClick={() => setGuideOpen(false)} aria-label="Close guide">Close guide</button>
          <div className="guide-visual" aria-hidden="true"><Image src="/reality-pet.png" alt="" fill priority sizes="(max-width: 800px) 100vw, 48vw" /></div>
          <div className="guide-panel">
            <div className="guide-id"><span className="guide-pulse" /><span>ROBOTIC PET // REALITY GUIDE</span></div>
      ...
      <section className="story-hero" id="top">
        <div className="story-hero-copy">
          ...
          <div className="story-hero-actions"><a href="#story">Follow the discovery <span>↓</span></a><button onClick={() => { setGuideStep(0); setGuideOpen(true); }}>Open the robotic guide</button></div>
        </div>
        <aside className="story-pet" aria-label="Robotic pet field guide">
          <div className="story-pet-image"><Image src="/reality-pet.png" alt="Reality robotic guide" fill priority sizes="(max-width: 880px) 100vw, 36vw" /></div>
          <div className="story-pet-copy"><span>ROBOTIC PET / FIELD GUIDE</span><p>“Do not start with our answer. Follow the evidence that forced the question to change.”</p></div>
        </aside>
      ...
      <section className="story-engine" id="story">
        ...
        <article className="chapter-canvas" key={chapter.id}>
          ...
          <footer>
            <div className="chapter-pet"><span className="mini-pet"><Image src="/reality-pet.png" alt="" fill sizes="52px" /></span><p><b>GUIDE NOTE</b>{chapter.guideNote}</p></div>
